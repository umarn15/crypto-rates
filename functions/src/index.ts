...
//             params: {
//               symbols: symbols.join(','),
//               referenceCurrencyUuid: 'yhjMzLPhuIDl' // USD reference
//             }
//           }
//         );
//
...
//         const priceMap = new Map<string, number>();
//
//         for (const coin of response.data.data.coins) {
//           if (coin && coin.symbol && coin.price) {
//             let price = parseFloat(coin.price);
//
//             if (coin.symbol === 'BTC') {
//               console.log('Original BTC price:', coin.price);
//               if (coin.btcPrice) {
//                 price = parseFloat(coin.btcPrice);
//               } else if (coin.usdPrice) {
//                 price = parseFloat(coin.usdPrice);
//               }
//               // Additional safety check for BTC
//               if (price < 1000) { // Assuming BTC won't be less than $1000
//                 console.error('Suspiciously low BTC price detected:', price);
//                 continue;
//               }
//             }
//
...
//               console.log(`Parsed price for ${coin.symbol}: ${price}`);
//             } else {
//               console.error(`Invalid price format for ${coin.symbol}: Original price: ${coin.price}, Parsed price: ${price}`);
//             }
//           } else {
...
//           console.log(`Final price check for ${symbol}: ${price}`);
//           if (!price || price <= 0) {
//             console.error(`Invalid or missing price for ${symbol}`);
//           }
//         });
//
//         return priceMap;
...
//   }
//
//   // Alternative BTC-specific approach (commented out)
//   /*
//   static async fetchBTCPrice(): Promise<number> {
//     for (let i = 0; i < this.apiKeys.length; i++) {
//       try {
...
//
//     /* Alternative approach with separate BTC handling
//     const priceMap = await ApiKeyManager.fetchWithKeyRotation(
//       symbols.filter(s => s !== 'BTC')
//     );
//
